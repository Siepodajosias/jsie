...
if [ -f "$TESTS_FOLDER\\$1" ]; then
	...
	if [[ $LAUNCH == true ]]; then
		...
		information_message "Lancement : npx playwright test $SPEC$LISTE_PARAMS" --config=playwright.edge.prod.config.ts 

		# ----------------------------------------------------------------------
		cd "c:\pw\playwright"
		retour=$(npx playwright test ${SPEC}${LISTE_PARAMS} --config=playwright.edge.prod.config.ts)
		# ----------------------------------------------------------------------
