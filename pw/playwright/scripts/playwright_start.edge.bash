	...
	if [[ $LAUNCH == true ]]; then
		...
		if [[ $NB_TIRS > 0 ]]; then						# On souhaite lancer le test NB_TIRS fois pour s'assurer de sa stabilité
			...
			do  
				...
				retour=$(npx playwright test ${SPEC}${LISTE_PARAMS} --quiet --config=playwright.edge.config.ts --reporter=dot)
			...
			npx playwright test $SPEC$LISTE_PARAMS --config=playwright.edge.config.ts
