       ...
       test.describe('Onglet [UNITES DE TRANSPORT]', async() => {
            ...
            test.describe ('Popin ' + sNomPopin.toUpperCase() + ']', async() => {
                ...
                test ('InputField [NOM] = "' + sNomUnite + '"', async () => {
                    await fonction.sendKeys(pageref.pPinputAddNom, sNomUnite, false, 'Nom Unité');
                    oData.sUniteTransport = sNomUnite;
                    log.set('Nom : ' + sNomUnite);
                })

                test ('InputField [VOLUME] = "' + rVolume + '"', async () => {
                    await fonction.sendKeys(pageref.pPinputAddVolume, rVolume, false, 'Volume');
                    log.set('Volume : ' + rVolume);
                })

                test ('Button [OK] - Click', async () => {
                    await fonction.clickAndWait(pageref.pPbuttonAddOk, page);
                    var isVisible = await pageref.errorMessage.isVisible();
                    if(isVisible){
                        var sErrorMessage= await pageref.errorMessage.locator('div').textContent();
                            sErrorMessage= sErrorMessage.slice(0,6);
                        if(sErrorMessage == "[4119]"){
                            await fonction.clickElement(pageref.pPlinkAddAnnuler);
                        }
                    }
                })
