...
const xRefTest      = "QUA_ART_AQU";
const xDescription  = "Associer un questionnaire à un article/ famille / sous-famille";
const xIdTest       =  5189;
const xVersion      = '3.4';
...
var info:CartoucheInfo = {
	...
	idTest      : xIdTest,
	help        : [],
	params      : ['nomQuestionnaire','codeArticle','rayon'],
	fileName    : __filename
};
...
var sNomQuestionnaire                   = fonction.getInitParam('nomQuestionnaire', 'TA_QuestionnaireArrivage_');
var sCodeArticle                        = fonction.getInitParam('codeArticle','5254');//data.sCodeArticle L1N8
var sRayon                              = fonction.getInitParam('rayon', 'Fruits et légumes');
    ...
    test.describe ('Page [REFERENTIEL]', async () => {
		...
		test.describe ('Onglet['+sNomOnglet.toUpperCase()+']', async () =>  {
            ...
            test.describe ('Popin [' + sNomPopin + '] - Click', async () => {
                ...
                test ('ListBox [QUESTIONNAIRE] = "' + sNomQuestionnaire + '"', async () => {
                    await fonction.clickElement(pageReferentielArticles.pPMaListBoxArticle);
                    await fonction.sendKeys(menu.inputMultiple, sNomQuestionnaire, false, 'Nom Questionnaire');
                    await fonction.clickElement(pageReferentielArticles.pPCheckBoxArticle.nth(0));        
                })
