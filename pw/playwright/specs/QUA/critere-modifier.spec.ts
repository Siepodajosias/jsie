...
const xRefTest      = "QUA_DET_MCR";
const xDescription  = "Modifier un critère";
const xIdTest       =  6664;
const xVersion      = '3.2';
...
var info:CartoucheInfo = {
	...
	idTest      : xIdTest,
	help        : [],
	params      : ['nomQuestionnaire','rubrique','groupeRubrique','rayon','objet'],
	fileName    : __filename
};
...
const sGroupeRubrique                     = fonction.getInitParam('groupeRubrique','Bonnes Pratiques d\'Hygiène (BPH)'); 
...
test.describe.serial ('[' + xRefTest + ']',  () =>  {
    ...
    test.describe ('Page [' + sNomPage.toUpperCase() + ']', async () => {
		...
		test.describe ('Onglet [' + sNomOnglet.toUpperCase() + ']', async () =>  {
            ...
            var sNomPopin:string = 'Modification d\'un critère';
            test.describe('Popin [' + sNomPopin.toUpperCase() + ']', async () => {

                test ('Button [MODIFIER CRITERE] - Click', async () => {
                    await fonction.clickAndWait(pageReferentielDetailQuestion.buttonModifierCritere, page);
                }) 
