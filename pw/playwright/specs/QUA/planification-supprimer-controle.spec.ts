...
const xRefTest       = "QUA_PAR_SPL";
const xDescription   = "Supprimer un Contrôle planifié";
const xIdTest        =  5219;
const xVersion       = '3.1';
...
test.describe.serial ('[' + xRefTest + ']',  () =>  {
    ...
    test.describe ('Page [' + sNomPage.toUpperCase() + ']', async () => {
        ...
        test.describe ('PLANIFICATION SEMAINE 1', async () => {
        ...
        test ('Button [SUPPRIMER] - Click', async () => {
            const tdFamilleASupprimer = pagePlanification.dataGridPlanifFamille.locator('tr').filter({hasText:sFamille});
            await tdFamilleASupprimer.first().hover();
            await fonction.clickAndWait(tdFamilleASupprimer.first().locator('em.fas.fa-times'), page);
        })

        test ('Button [ENREGISTRER] #1 - Click', async () => {
            await fonction.clickAndWait(pagePlanification.buttonEnregistrer, page);
        })  
