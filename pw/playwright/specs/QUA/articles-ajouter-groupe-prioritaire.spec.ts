...
const xRefTest      = "QUA_ART_AGP";
const xDescription  = "Ajouter un article au groupe prioritaire";
const xIdTest       =  5190;
const xVersion      = '3.3';
    ...
    test.describe ('Page [REFERENTIEL]', async () => {
		...
		test.describe ('Onglet['+sNomOnglet.toUpperCase()+']', async () =>  {
            ...
            test.describe ('Popin [' + sNomPopin + '] - Click', async () => {
                ...
                test ('RadioButton [GROUPE PRIORITAIRE] - Click', async () => {  
                    if ( await pageReferentielArticles.pPMaRadioButtonGroupePrio.locator('.p-inputswitch-checked').count() > 0){
                        log.set('L\'article ' + sCodeArticle + ' est déjà dans le groupe prioritaire');
                        await fonction.clickElement(pageReferentielArticles.pPMaButtonAnnuler);
                    } else {
                        log.set('L\'article ' + sCodeArticle + 'n\' pas dans le groupe prioritaire');
                        await fonction.clickElement(pageReferentielArticles.pPMaRadioButtonGroupePrio); 
