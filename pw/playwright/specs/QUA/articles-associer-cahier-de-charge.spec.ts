...
const xRefTest      = "QUA_ART_ACD";
const xDescription  = "Associer un cahier de charges à un article/ famille / sous-famille";
const xIdTest       =  5188;
const xVersion      = '3.3';
...
const aCahierDeCharges                      = fonction.getLocalConfig('aCahierDeCharges');
const sFilleArticle                         = path.join(__dirname + '../' + aCahierDeCharges[0]);
const sFilleFamille                         = path.join(__dirname + '../' + aCahierDeCharges[1]);
const sFilleSousFamille                     = path.join(__dirname + '../' + aCahierDeCharges[2]);
    ...
    test.describe ('Page [REFERENTIEL]', async () => {
		...
		test.describe ('Onglet['+sNomOnglet.toUpperCase()+']', async () =>  {
            ...
            test.describe ('Popin [' + sNomPopin + '] - Click', async () => {
                ...
                test ('Button [PACOURIR - AJOUTER UN CAHIER DE CHARGES A UN ARTICLES] - Click', async () => {
                    await pageReferentielArticles.pPMaInputParcourirArticle.setInputFiles(sFilleArticle);  
                })

                test ('Button [PACOURIR - AJOUTER UN CAHIER DE CHARGES A LA FAMILLE] - Click', async () => {
                    await pageReferentielArticles.pPMaInputParcourirFamille.setInputFiles(sFilleFamille); 
                })

                test ('Button [PACOURIR - AJOUTER UN CAHIER DE CHARGES A LA SOUS-FAMILLE] - Click', async () => {
                    await pageReferentielArticles.pPMaInputParcourirSousFamille.setInputFiles(sFilleSousFamille); 
                })
