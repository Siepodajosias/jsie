...
const xRefTest      = "QUA_QUE_GRU";
const xDescription  = "Gérer les rubriques - Ajout";
const xIdTest       =  4632;
const xVersion      = '3.1';
...
var info:CartoucheInfo = {
	...
	idTest      : xIdTest,
	help        : [],
	params      : ['groupeRubrique','rayon','objet'],
	fileName    : __filename
};
...
var sRubrique                              = fonction.getInitParam('groupeRubrique','TA_Groupe_Rubrique-'+ fonction.getToday()); 
		...
		test.describe ('Onglet [QUESTIONNAIRE]', async () =>  {
            ...
            test.describe ('Popin [' + sNomPopin.toUpperCase() + ']', async () => {
                ...
                test ('ListBox [OBJET]= "' + sObjet + '" - Select', async () => {
                  const sValeur= await pageReferentielQuestionnaires.pPGrListBoxObjetSelected.textContent(sObjet);
                  if(!sValeur){
                ...
                test ('InputField [NOUVELLE RUBRIQUE]= "'+ sRubrique +'"', async () => {
                    await pageReferentielQuestionnaires.pPGrInputNouvelleRubrique.pressSequentially(sRubrique,{ delay: 100 })
                    await fonction.wait(page, 250);
                })   
               
                test ('Button [AJOUTER RUBRIQUE] - Click', async () => {
                    await fonction.clickElement(pageReferentielQuestionnaires.pPGrButtonAjouterRubrique);          
                })
