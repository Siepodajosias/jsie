...
const xRefTest      = "QUA_DET_ACO";
const xDescription  = "Ajouter un critère de type O/N";
const xIdTest       =  6663;
const xVersion      = '3.0';
...
const sNomQuestionnaire                    = fonction.getInitParam('nomQuestionnaire', 'TA_QuestionnaireTypeOuiNon_'+ fonction.getToday());
...
const sTypeCritere                         = fonction.getInitParam('typeCritere', 'Oui / Non');
		...
		test.describe ('Onglet [' + sNomOnglet.toUpperCase() + ']', async () =>  {
            ...
            test.describe ('Popin [' + sNomPopin.toUpperCase() + '] #4', async () => {
                ...
                test ('InputField  [DESIGNATION CRITERE]', async () => {                   
                    await fonction.sendKeys(pageReferentielDetailQuestion.pPAcInputDesignationCritere,'critère Oui / Non', false, 'Designation Critere');     
                })
