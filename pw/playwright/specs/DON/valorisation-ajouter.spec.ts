 ...
 *  
 */
const xRefTest 		= "DON_REC_AJV";
const xDescription 	= "Ajouter une valorisation";
const xIdTest 		= 5992;
const xVersion 		= '3.3';
...
var info: CartoucheInfo = {
	...
	idTest		: xIdTest,
	help		: [],
	params		: ['societeDonatrice', 'poids', 'valorisation'],
	fileName	: __filename
}
...
var iPoidsInitial = fonction.getInitParam('poids', '10');
var iValorisationInitial = fonction.getInitParam('valorisation', '10');
...
test.describe.serial ('[' + xRefTest + ']', () => {
	...
	test.describe('Page [DONS]', async () => {
		...
		test.describe('Onglet [' + sNomOnglet + ']', async () => {
			...
			test('Button [AJOUTER UNE VALORISATION] - Click', async () => {
				await fonction.clickAndWait(pageDonsRecap.buttonAjouterValorisation, page);
			})

			var sNomPopin: string = 'AJOUTER UNE VALORISATION';
			test.describe('Popin [' + sNomPopin + ']', async () => {

				test('Button [AJOUTER UN GROUPE ARTICLE] - Click', async () => {
					await fonction.clickAndWait(pageDonsRecap.pPavButtonGroupeArticle, page);
				})
				...
				test('Ajouter [POIDS INITIAL] = "' + iPoidsInitial + '"', async () => {
					await pageDonsRecap.dataGridPoidsInitial.pressSequentially(iPoidsInitial);
				})

				test('Ajouter [VALORISATION INITIAL] = "' + iValorisationInitial + '"', async () => {
					await pageDonsRecap.dataGridValorisationInitial.pressSequentially(iValorisationInitial);
				})

				test('Button [ENREGISTRER] - Click', async () => {
					await fonction.clickAndWait(pageDonsRecap.pPavButtonEnregistrer, page);
				})

				test('** Wait Until Spinner Off **', async () => {
					await fonction.waitForSpinner(pageDonsRecap.pPspinner);
				})
