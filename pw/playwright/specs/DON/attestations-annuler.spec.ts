 ...
 *  
 */
const xRefTest      = "DON_SAT_ANR";
const xDescription  = "Annuler Réceptions";
const xIdTest       = 4731;
const xVersion      = '3.0';
	...
	test.describe ('Page [BENEFICIAIRES]', async () => {
		...
		test.describe ('Onglet [' + sNomOnglet + ']', async () => {
			...
			test('Header [STATUS] - Click', async () => {
				await pageBenefSuiviAttest.dataGridStatut.dblclick();
			})
			...
			var sNomPopin:string = 'ANNULER RECEPTION';
			test.describe ('Popin [' + sNomPopin + ']', async () => {

				test('Button [ANNULER RECEPTION] - Click', async () => {
				...
				test('Button [CONFIRMER] - Click', async () => {
					await fonction.clickAndWait(pageBenefSuiviAttest.pParButtonConfirmer, page);
				}) 
